...
def initiate_signature_request(
    ...
    if len(signature_request.signatories) == 1:
        ...
        if signature_request_data.ordered_signers:
            ...
            if email_response and email_response.status_code == 250:
                    ...
                    if notification_email_response:
                        error_detail += f"""
                            Server responded with status: {
                                notification_email_response.status_code
                            }.
                        """
                ...
                error_message = f"""
                    Failed to send email: {
                        email_response.status_text if email_response else 'No response'
                    }
                """
        ...
        else:  # if signatories not ordred
            for signatory in signature_request_data.signatories:
                ...
                if email_response and email_response.status_code == 250:
                    # Email sent successfully, update document status
                    for document_id in signature_request_data.documents:
                        document = document_crud.get_document_by_id(db, document_id)
                        if document:
                            document.status = DocumentStatus.SENT_FOR_SIGNATURE
                            db.add(document)
...
@router.post(
    "/test", response_model=SignatureRequestRead, status_code=status.HTTP_201_CREATED
)
def initiate_signature_request_2(
    request: Request,
    signature_request: SignatureRequestCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
